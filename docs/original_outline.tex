...
1. Cột chấm (Vector) morph thành Đường cong (Hàm). 
2. Lưới ô vuông (Ma trận W) morph thành Heatmap 2D (Kernel $\kappa$). 
3. Ký hiệu $\sum$ morph thành $\int$.
...
& Các object \manim{Transform} mượt mà. Không dùng text liệt kê. Nhấn mạnh sự tương đồng về mặt cấu trúc (structural isomorphism). \\ \hline
\textbf{[9:45--10:15]} \newline 
\textbf{Nghịch lý Làm mịn:} 
Hiện một hàm có gai nhọn (shock wave). Đi qua Khối tích phân $\to$ Output bị nhòe (mất gai). 
Hiện mũi tên Bypass (Residual) nối thẳng Input $\to$ Output. 
Kết quả: Gai nhọn được phục hồi.
& \textit{"Nhưng tích phân mang theo một nghịch lý: nó là một bộ lọc thông thấp, có xu hướng làm mịn và xóa sổ các chi tiết sắc nét như sóng xung kích. Đây là lý do Residual Connection không chỉ là một thủ thuật, mà là một yêu cầu sinh tồn. Nó bơm thẳng tín hiệu gốc bypass qua lõi tích phân, giữ lại những chi tiết vật lý quan trọng nhất."} 
& Hàm sắc nét \manim{Create}. Qua kernel \manim{Transform} thành hàm nhòe. Mũi tên Residual \manim{GrowArrow} phát sáng. Hàm cuối cùng \manim{Flash} lại độ sắc nét. \\ \hline
\end{tabularx}
...
Input là 1 đường cong vật lý. 
Encoder (Pointwise) "ép" nó thành một chồng dày các đường cong đặc trưng (latent channels). 
Chồng này đi qua các "Thấu kính Tích phân" (Operator Layers). 
Decoder (Pointwise) nén chồng đó lại thành 1 đường cong output. 
...
& Đường cong đơn \manim{Transform} thành chồng lớp (stack). 
Stack trượt qua các khối \manim{IntegralLens}. 
Stack nén lại thành đường cong đơn. 
Text overlay \manim{Write} sync VO. \\ \hline
\textbf{[10:45--11:15]} \newline 
\textbf{Bẫy Rời rạc hóa (Discretization Error):} 
Chia đôi màn hình. 
Trên: Hàm trơn (Smooth). Lưới $N$ tăng $\to$ Riemann sum ôm sát đường cong. 
Dưới: Hàm Shock (gián đoạn). Lưới $N$ tăng $\to$ vẫn bị sai số lớn ở điểm gãy (highlight đỏ). 
Hiện công thức $O(N^{-s})$.
...
& \manim{RiemannSum} animation cho 2 hàm song song. 
Highlight vùng sai số (màu đỏ) ở điểm Shock. 
Công thức \manim{Write} và nhấn mạnh $s$. \\ \hline
\end{tabularx}
