...
% ================= SCENE 1.2 =================
\subsection{Scene 1.2: Plot twist -- Thế giới khoa học: dữ liệu là hàm số}
...
\textbf{[0:45--1:00]} \newline Khung $\mathbb{R}^n$ shatter effect $\to$ lộ cầu Trái Đất với 3 lớp trường chồng lên: nhiệt độ (đỏ), gió (xanh), áp suất (vàng). 
& \textit{"Tuy nhiên, nếu chúng ta bước ra khỏi Khoa học Máy tính và đi dạo qua các khoa khác trong trường đại học, bức tranh thực tế sẽ thay đổi hoàn toàn:"} 
& Shatter: \manim{FadeOut} $\mathbb{R}^n$ + \manim{Create} globe. Layers fade-in sequentially (0.5s each). \\ \hline

\textbf{[1:00--1:25]} \newline Lần lượt hiện 4 icon + mini-animation: \textcolor{orange}{Da liễu} (gradient da), \textcolor{orange}{Địa vật lý} (sóng 3D), \textcolor{orange}{CFD} (dòng khí), \textcolor{orange}{Khí hậu} (100+ hàm trên cầu). 
& \textit{"Da liễu nghiên cứu mô da trên bề mặt liên tục. Địa vật lý phân tích hàm 3D mô tả sóng địa chấn. Cơ khí tính toán trường vector 4D. Khí hậu mô phỏng hàng trăm hàm trên cầu. Ở đó, dữ liệu của họ không phải là ảnh hay token. Dữ liệu của họ là \textbf{hàm số}."} 
& Mỗi icon: \manim{Scale} in (0.3s) $\to$ zoom $\to$ surface/field \manim{Create} $\to$ label \manim{Write}. Sync mỗi domain với 6-7s VO. \\ \hline

\textbf{[1:25--1:45]} \newline Zoom cực gần vào 1 điểm trên cầu $\to$ vẫn thấy gradient liên tục, \textbf{không pixel}. Overlay: \textbf{Data = Functions}. 
& \textit{"Và đây là điểm then chốt: khi ta cố ép một mô hình Deep Learning truyền thống xử lý dữ liệu này bằng cách rời rạc hóa thành lưới 64x64..."} 
& Infinite zoom: \manim{CameraFrame} move + \manim{Scale} field. Emphasize continuity with smooth gradient, no grid lines. \\ \hline
...
% ================= SCENE 1.4 =================
\subsection{Scene 1.4: Traditional Solvers \& 4 hạn chế}
...
\hline
\textbf{Visual (Manim)} & \textbf{Audio / Voiceover} & \textbf{Ghi chú Animation} \\ \hline
\textbf{[2:30--2:50]} \newline Hiện PDE: $-\nabla \cdot (a\nabla u)=f$ với từng thành phần highlight theo lời. Flowchart: PDE $\to$ Finite Diff $\to$ Fixed Grid. & \textit{"Trước khi có Neural Operators, các nhà khoa học giải quyết những bài toán này bằng phương pháp số truyền thống. Quy trình chuẩn là: mô hình hóa hiện tượng bằng Phương trình Đạo hàm Riêng (PDE) như Navier-Stokes cho chất lưu hay Schrödinger cho cơ học lượng tử, sau đó dùng sai phân hữu hạn hoặc phần tử hữu hạn để giải trên một lưới cố định."} & Equation build-up: each term \manim{Write} + color highlight. Flowchart: \manim{Arrow} nối nodes sequentially. \\ \hline
\textbf{[2:50--3:10]} \newline Lưới mịn dần: $16^2 \to 32^2 \to 64^2 \to 128^2$. Đồng hồ bên cạnh chạy nhanh dần $\to$ "bùng nổ" icon. & \textit{"Cách tiếp cận này rất mạnh mẽ nhưng tồn tại những nút thắt khó gỡ: Sai số tích lũy... Bùng nổ chi phí tính toán: Khi độ phân giải tăng lên để đạt độ chính xác cần thiết, chi phí tính toán tăng theo lũy thừa, vượt quá khả năng chi trả của nhiều tổ chức."} & Grid refinement: \manim{Transform} with increasing density. Timer: \manim{ValueTracker} exponential speed-up $\to$ explosion effect. \\ \hline
\textbf{[3:10--3:30]} \newline 4 icon hiện lần lượt, mỗi icon có mini-animation: \textcolor{red}{ Sai số tham số hóa}, \textcolor{red}{Chi phí bùng nổ}, \textcolor{red}{Rào cản chuyên môn}, \textcolor{red}{Không khả vi}. & \textit{"Và còn những nút thắt khó gỡ: sai số tích lũy do tham số hóa, chi phí tính toán tăng theo lũy thừa, rào cản chuyên môn, tính không khả vi khiến việc đưa dữ liệu thực tế ngược lại vào mô hình trở nên cực kỳ phức tạp. Đừng hiểu lầm, các bộ giải truyền thống là thành tựu vĩ đại. Mục tiêu không phải thay thế, mà bổ sung ML vào toolbox."} & Icons: \manim{FadeIn} + \manim{Scale} (0.5s each). Micro-animation: cloud distortion, money chart, book stack, gradient blocked. \\ \hline
\end{tabularx}
...
\textbf{[3:55--4:00]} \newline Fade to black + text: \textbf{Section 2: Thế giới hàm số}. & \textit{(nhạc chuyển cảnh, pause ngắn)} & Clean \manim{FadeOut} $\to$ text \manim{Write} centered. Hold 1s before cut. \\ \hline
\end{tabularx}
...
\subsection{Scene 2.1: Bản chất dữ liệu khoa học -- Mọi thứ đều là hàm số}
...
\hline
\textbf{Visual (Manim)} & \textbf{Audio / Voiceover} & \textbf{Ghi chú Animation} \\ \hline
\textbf{[4:00--4:30]} \newline Hiện cầu Trái Đất với các lớp dữ liệu ERA5 (nhiệt, gió, áp suất). Mũi tên nối "Trạng thái hôm nay" $\to$ "Trạng thái ngày mai". Overlay: \textbf{Hàm $\to$ Hàm}. 
& \textit{"Trước khi đi sâu vào định nghĩa Neural Operator, hãy quan sát bản chất dữ liệu khoa học. Dự báo thời tiết: nhiệt độ, vận tốc gió, áp suất tại mỗi điểm trên mặt cầu đều là các hàm số. Bài toán dự báo thực chất là ánh xạ từ tập hợp hàm số hôm nay sang tập hợp hàm số ngày mai. Đạo hàm và tích phân của chúng phải tuân định luật vật lý, cho ra năng lượng hay moment động lượng."} 
& Globe rotate chậm. Layers \manim{FadeIn} tuần tự. Arrow \manim{Create} nối 2 trạng thái. Text overlay \manim{Write} sync với từ khóa "ánh xạ". \\ \hline
\textbf{[4:30--4:50]} \newline Hiện khối 3D dưới lòng đất với sóng lan truyền. Chuyển cảnh sang geometry xe/máy bay + trường áp suất bao quanh. 
& \textit{"Địa chấn học: trường vận tốc ba chiều $a(x)$ mô tả sóng lan truyền. Bài toán ngược quan trọng là từ quan sát bề mặt suy ra cấu trúc bên trong. Khí động học: hình dạng vật thể là hàm geometry, output là trường áp suất và vận tốc thỏa mãn Navier-Stokes. Neural Operator cho kết quả tương đương solver truyền thống chỉ trong vài giây."} 
& 3D block \manim{Create} với wave propagation effect. Geometry surface \manim{Transform} sang flow field. Highlight PDE constraint subtly. \\ \hline
\textbf{[4:50--5:00]} \newline Hiện quỹ đạo nguyên tử liên tục. Text overlay: \textbf{Input = Hàm, Output = Hàm, Ràng buộc vật lý liên tục}. 
& \textit{"Động lực học phân tử: nguyên tử chuyển động liên tục theo Schrödinger. Quỹ đạo không phải dãy điểm rời rạc mà là hàm số liên tục. Điểm chung của tất cả bài toán này: dữ liệu vào là hàm số, dữ liệu ra là hàm số, và output phải thỏa mãn các ràng buộc vật lý liên tục."} 
& Particle trajectory \manim{ParametricCurve} moving smoothly. Checklist text \manim{FadeIn} từng dòng. Sync VO nhịp cuối. \\ \hline
\end{tabularx}
...
\hline
\textbf{Visual (Manim)} & \textbf{Audio / Voiceover} & \textbf{Ghi chú Animation} \\ \hline
\textbf{[5:40--6:10]} \newline Màn hình chia đôi: Trái $\mathbb{R}^n \to \mathbb{R}^m$ (vector rời rạc). Phải $\mathcal{A} \to \mathcal{U}$ (trường hàm liên tục). 
& \textit{"Để hiểu Neural Operator, ta so sánh với Deep Learning cổ điển. DL truyền thống học hàm số $f_{\theta}: \mathbb{R}^n \to \mathbb{R}^m$ -- ánh xạ giữa các vector hữu hạn chiều. Operator Learning muốn học toán tử $G_{\theta}: \mathcal{A} \to \mathcal{U}$ -- ánh xạ giữa các không gian hàm vô hạn chiều."} 
& Split screen \manim{VGroup}. Left side: discrete dots/vectors. Right side: smooth surfaces/fields. Labels \manim{Write} sync with VO. \\ \hline
\textbf{[6:10--6:40]} \newline Hiện phương trình Darcy Flow: $-\nabla \cdot (a(x) \nabla u(x)) = f(x)$. Highlight $a(x)$ (xanh) $\to$ $u(x)$ (tím). Mũi tên học được $G$ nối hai hàm. 
& \textit{"Lấy ví dụ Darcy Flow: $-\nabla \cdot (a(x) \nabla u(x)) = f(x)$. Trong đó $a(x)$ là hệ số khuếch tán, $u(x)$ là nghiệm. Thay vì giải số phức tạp cho mỗi $a$ mới, ta muốn học toán tử $G$ sao cho $G(a) \approx u$."} 
& Equation build-up term by term \manim{Write}. Highlight colors applied to variables. Operator $G$ \manim{Create} as curved arrow between functions. \\ \hline
\end{tabularx}
